...
function iconSvgSafe(type) {
  if (type === "library") {
    return '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="M5 4.5h5.5A2.5 2.5 0 0 1 13 7v13H7.2A2.2 2.2 0 0 0 5 22z"></path><path d="M19 4.5h-5.5A2.5 2.5 0 0 0 11 7v13h5.8A2.2 2.2 0 0 1 19 22z"></path></svg>';
  }
  if (type === "park") {
    return '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21v-6"></path><path d="M12 15c-3.5 0-6-2.4-6-5.8C6 6.1 8.2 4 11.2 4c1.8 0 3 .8 3.8 2.1C15.7 5 17 4.4 18.5 4.4 20.9 4.4 23 6.5 23 9c0 3.3-2.4 6-6.1 6"></path></svg>';
  }
  return '<svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12a7 7 0 1 0 14 0 7 7 0 0 0-14 0"></path><path d="M9.5 12.5c.7 1.3 2 2 2.5 2s1.8-.7 2.5-2"></path><path d="M9 10h.01"></path><path d="M15 10h.01"></path></svg>';
}

function safeSpaceType(place) {
  const text = `${place["Sub Theme"] || ""} ${place.Theme || ""}`.toLowerCase();
  if (text.includes("library")) return "library";
  if (text.includes("park") || text.includes("garden") || text.includes("reserve")) return "park";
  return "quiet";
}

function safeSpaceIcon(type = "quiet") {
  return L.divIcon({
    className: "safe-space-icon-wrapper",
    html: `<div class="safe-space-marker ${type}" aria-hidden="true">${iconSvgSafe(type)}</div>`,
    iconSize: [32, 32],
    iconAnchor: [16, 16],
    popupAnchor: [0, -12]
  });
...
function drawCrowdCoverage(routeCoordinates) {
  ...
  nearbySensors.forEach(sensor => {
    if (sensor.status === "no_data") return;
    ...
    const forecastLowText = sensor.forecast_low_threshold == null ? "Unavailable" : `${Math.round(Number(sensor.forecast_low_threshold))}`;
    ...
    coverage.bindPopup(`
      ...
      Next-hour 33rd percentile: ${forecastLowText}<br>
      Next-hour 66th percentile: ${forecastHighText}<br>
      Prediction method: ${methodText}<br>
